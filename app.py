...
def certificates():
    ...
    return render_template('BarangayAdmin/certificates.html',
...
def employee_clearance(employee_id):
    ...
    response.headers['Content-Disposition'] = (
        f'attachment; filename="clearance_{employee.id}.pdf"'
    )
    return response
...
def residency_certificate(residence_id):
    ...
    html = render_template(
        ...
        issued_date   = datetime.now(timezone.utc),
        purpose       = purpose,
        barangay_name = "Barangay Salimbao",        # ← change to your barangay
        municipality  = "General Santos City",       # ← change to your municipality
        captain_name  = "Hon. Ricardo Cruz Jr.",     # ← change to your captain
    )
    ...
    response.headers['Content-Type']        = 'application/pdf'
    response.headers['Content-Disposition'] = (
        f'attachment; filename="residency_{residence_id}.pdf"'
    )
    return response
